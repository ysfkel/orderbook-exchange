...
pub enum ProgramError {
    ...
    #[error("Aeron initialization error: {0}")]
    InitAeronError(#[from] AeronCError),
}
...
impl From<ProgramError> for Status {
    fn from(err: ProgramError) -> Self {
        match err {
            ...
            ProgramError::InitAeronError(e) => {
                error!("Aeron initialization error: {}", e);
                Status::internal("Failed to initialize Aeron")
            }
        }
    }
}
