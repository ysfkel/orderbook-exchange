...
use common::{
    queue::{QueueConsumer, QueueRecvError, RingBufferConsumer}, types::AcceptedOrder
};
use transport::{AeronTransport, Publisher, PublishError};
...
impl  AcceptedOrderPublisher { 
   ...
   fn run_inner<T>(&mut self, publisher: &OrderPublisher<T>) -> Result<(), PublishError> where T: Publisher, T::Error: Into<PublishError> {
