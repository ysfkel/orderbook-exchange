...
use common::types::CreateOrderMessage;
...
pub fn run() {
    ...
    let fragment_handler = AeronTransport::build_fragment_handler(|msg: &[u8]| {
        match CreateOrderMessage::try_ref_from_bytes(msg) {
            Ok(order) => {
                 println!(
                    "order received: price={}, quantity={}",
                    order.body.price, order.body.quantity
                 );
            }
            Err(e) => {
                eprintln!("failed to parse CreateOrderMessage {:?}", e)
            }
        }
    })
